#####----- 
mff=function(theta1Input,theta2Input,weightInput,M,dateInput){
...
frank.cop2 <- frankCopula(iTau(frankCopula(), theta2Input), dim = d)

rN=ifelse((M*weightInput)<1,1,floor(M*weightInput))

U=t(rbind(rCopula(rN, frank.cop1 ),rCopula(M-rN,frank.cop2 ))[sample(M,M,replace=FALSE),])
