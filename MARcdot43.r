t43=function(theta1Input,theta2Input,theta3Input,dF,M,dateInput){
#vTTrancheF13(1/115,1/116,1/116,13,10,c("2008/4/4"))
#theta1Input=.15
#theta2Input=.08
#weightInput=.03
#MInput=100
...
#theta3Input=.5
#M=10
#theta[1] <-copGumbel@iTau(theta1Input)
...
rho2=theta2Input
rho3=theta3Input
s1=matrix(rho3,30,30)+(1-rho3)*diag(30)
s2=matrix(rho3,25,25)+(1-rho3)*diag(25)
s3=matrix(rho3,20,20)+(1-rho3)*diag(20)
s4=matrix(rho3,20,20)+(1-rho3)*diag(20)
s5=matrix(rho3,20,20)+(1-rho3)*diag(20)
s6=matrix(rho3,10,10)+(1-rho3)*diag(10)
vsigma=matrix(rho2,126,126)
vsigma[1:30,1:30]=s1
...
vsigma[126,1:126]=c(seq(rho1,rho1,length.out=125),1)
vsigma[1:126, 126]=c(seq(rho1,rho1,length.out=125),1)
vsigma
dF=floor(dF)
#t1.cop=tCopula(dim = d, dispstr = "ex",df = dF, df.fixed = TRUE)
#---1) 2)Compute tao  
t.cop <- tCopula(P2p(vsigma), dim = 126, df = dF, disp = "un", df.fixed = TRUE)  
#U <- rCopula(M, norm.cop)  
#U=t(rCopula(M, norm.cop))
#---1) 2)Compute tao  
#norm.cop <- normalCopula(P2p(vsigma), dim = 126, disp = "un")  
#U <- rCopula(M, norm.cop)  
#U=t(rCopula(M, norm.cop))
