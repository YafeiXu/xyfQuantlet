#####-----vMixedGsCTranche
mtt=function(theta1Input,theta2Input,weightInput,M,dateInput){
...
lambdaVector=read.csv("/defIntensity.csv")
...
t.cop1 <- tCopula(c(iTau(t1.cop,theta1Input)), dim = d, dispstr = "ex",df = 3, df.fixed = TRUE)

t2.cop=tCopula(dim = d, dispstr = "ex",df = 3, df.fixed = TRUE)

t.cop2 <- tCopula(c(iTau(t2.cop,theta2Input)), dim = d, dispstr = "ex",df = 3, df.fixed = TRUE)



rN=ifelse((M*weightInput)<1,1,floor(M*weightInput))

U=t(rbind(rCopula(rN, t.cop1 ),rCopula(M-rN, t.cop2))[sample(M,M,replace=FALSE),])
