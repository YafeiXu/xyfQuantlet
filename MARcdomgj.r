#####----- 
mgj=function(theta1Input,theta2Input,weightInput,M,dateInput){
...
joe.cop2 <- joeCopula(iTau(joeCopula(), theta2Input), dim = d)


rN=ifelse((M*weightInput)<1,1,floor(M*weightInput))

U=t(rbind(rCopula(rN, gumbel.cop1),rCopula(M-rN,joe.cop2))[sample(M,M,replace=FALSE),])
