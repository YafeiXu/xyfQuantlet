#####----- 
mcc=function(theta1Input,theta2Input,weightInput,M,dateInput){
...
clayton.cop1 <- claytonCopula(iTau(claytonCopula(), theta1Input), dim = d)
clayton.cop2 <- claytonCopula(iTau(claytonCopula(), theta2Input), dim = d)


rN=ifelse((M*weightInput)<1,1,floor(M*weightInput))

U=t(rbind(rCopula(rN, clayton.cop1 ),rCopula(M-rN,clayton.cop2))[sample(M,M,replace=FALSE),])
