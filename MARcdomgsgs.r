#####----- 
mgsgs=function(theta1Input,theta2Input,weightInput,M,dateInput){
...
#norm.cop <- normalCopula(theta1Input, dim = 125, dispstr = "ex") #  
#t1.cop=tCopula(dim = d, dispstr = "ex",df = 3, df.fixed = TRUE)
#t.cop <- tCopula(c(iTau(t1.cop,theta2Input)), dim = d, dispstr = "ex",df = 3, df.fixed = TRUE)
#I <- runif(M)<weightInput
   
#U=matrix(NA,M,d)
#for(i in 1:M){
#if(I[i]){
#U[i,]=rCopula(1, norm.cop)
...
norm.cop <- normalCopula(theta1Input, dim = 125, dispstr = "ex") #  
norm.cop2 <- normalCopula(theta2Input, dim = 125, dispstr = "ex") #  
#t1.cop=tCopula(dim = d, dispstr = "ex",df = 3, df.fixed = TRUE)
#t.cop <- tCopula(c(iTau(t1.cop,theta2Input)), dim = d, dispstr = "ex",df = 3, df.fixed = TRUE)
rN=ifelse((M*weightInput)<1,1,floor(M*weightInput))
#rCopNorm=rCopula(rN, norm.cop)
#rCopT=rCopula(M-rN, t.cop)
U=t(rbind(rCopula(rN, norm.cop),rCopula(M-rN, norm.cop2))[sample(M,M,replace=FALSE),])
#U=rbind(rCopNorm,rCopT)
#U=U[sample(M,M,replace=FALSE),]
#U=t(U)
