#####----- 
mjj=function(theta1Input,theta2Input,weightInput,M,dateInput){
...
joe.cop1 <- joeCopula(iTau(joeCopula(), theta1Input), dim = d)

joe.cop2 <- joeCopula(iTau(joeCopula(), theta2Input), dim = d)

rN=ifelse((M*weightInput)<1,1,floor(M*weightInput))

U=t(rbind(rCopula(rN, joe.cop1),rCopula(M-rN, joe.cop2))[sample(M,M,replace=FALSE),])
