gs1=function(theta1Input,theta2Input,M,dateInput){
#vNestedGaussianTrancheFormula8(-1/15,-1/16,10,c("2008/4/4"))
...
vsigma
#P2p(vsigma)
#normalCopula(P2p(vsigma), dim = 125, disp = "un")

#---1) 2)Compute tao 
norm.cop <- normalCopula(P2p(vsigma), dim = 125, disp = "un")  
#U <- rCopula(M, norm.cop)  

tao=(-1/lambda)*log(t(rCopula(M, norm.cop)))

#rNCop=pnorm(rmvnorm(M, mean = rep(0, 125), sigma = vsigma,method="chol"))

#

#copG <-onacopulaL("Gumbel", list(theta[1],NULL,list(list(theta[2],c(1:125)
#)
#)
#)
#)
