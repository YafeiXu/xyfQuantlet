#####----- 
gumbel=function(rhoInput,MInput,dateInput){
...
#---1) 2)Compute tao 计算违约时间  copulaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
#norm.cop <- normalCopula(c(rho), dim = d, dispstr = "ex") # 用copula抽取随机数
#t.cop=tCopula(c(0.5, 0.3), dim = 3, dispstr = "toep",df = 2, df.fixed = TRUE)
#clayton.cop <- claytonCopula(iTau(claytonCopula(), rho), dim = d)
#scatterplot3d(rCopula(1000, clayton.cop))
#t1.cop=tCopula(dim = d, dispstr = "ex",df = 4, df.fixed = TRUE)

#iTau(t.cop,rho)
gumbel.cop <- gumbelCopula(iTau(gumbelCopula(), rho), dim = d)
#t.cop <- tCopula(c(iTau(t1.cop,rho)), dim = d, dispstr = "ex",df = 4, df.fixed = TRUE)
U <- rCopula(M, gumbel.cop) # 完成default time矩阵赋值,抽出125个随机数Ui,i=1,...,125
U=t(U)
